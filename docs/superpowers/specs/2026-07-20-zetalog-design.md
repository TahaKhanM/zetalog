...
5. **Problem-stream conformity (W6 — problem-tampering defence):** a cheater who rewrites the _displayed_ problems (e.g. every problem becomes "2 + 2") yields an internally consistent stream that no other check catches; these rules reconstruct what Zetamac's generator can emit for the claimed settings.
   ...
   - **Operand marginals:** conditional on its op, every observable operand slot (add left/right; sub's subtrahend and answer; mul left/right; div's divisor and quotient) is uniform on its configured range, so P(value ≤ midpoint) is an exactly computed p₀ per slot. Over n ≥ 30 problems, K = (#slots)+1 two-sided Hoeffding tests under one union bound at the same family α = 1e-4 — one per slot plus one pooled test over all slot observations (the pooled test is what catches a balanced all-easy stream, e.g. 60 problems of "2 + k"/"k − 2"/"2 × k"/"2k ÷ 2" with small k, where per-slot samples are too few to ever fire). Deviation ⇒ `quarantined`. Same zero-spanning-divisor abstention as the mix rule.
   - **Repetition/entropy:** independent draws mean a single problem recurring beyond the birthday bound for the _claimed_ operand space (Poisson-tail union bound, α = 1e-4) ⇒ `quarantined`; runs from n ≥ 8 solved problems (the bound self-regulates at small n, so "2 + 2" × 19 — a plausible 30-second game — flags); narrow-range games are judged against their own small space and so are not flagged. Abstains on a zero-spanning divisor range (generator re-roll skews the draw law) and on a degenerate empty divisor space.
   ...
   **Residual limits:** these rules bound what a tamperer can pass; they cannot prove provenance. An attacker who replays a statistically faithful simulation of the true generator — correct ranges, uniform op mix and operand marginals, independent draws, human pacing — is undetectable by this layer **by construction**; defence in depth against that comes from the physiology, history, and rate rules plus human review of quarantined outliers. What IS covered: operands or operations impossible for the claimed settings (reject); a skewed operation mix; operands skewed toward the easy end (including balanced all-easy streams (median-balanced easy-extreme streams remain a known, documented gap — the next rung would be dispersion/joint tests, with diminishing returns)); excessive repetition of any single problem; unsolved problems being re-rendered.
